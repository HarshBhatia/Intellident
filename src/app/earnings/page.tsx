  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 pb-12">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
             <div className="flex items-center gap-2 cursor-pointer" onClick={() => router.push('/')}>
               ...
               <span className="text-gray-500 font-medium hidden sm:inline">Financials</span>
             </div>
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        {/* Controls Bar */}
        <div className="bg-white p-4 rounded border border-gray-200 shadow-sm mb-8 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
            {/* Tabs */}
            <div className="flex bg-gray-100 p-1 rounded-md">
                <button 
                    onClick={() => setActiveTab('categories')}
                    className={`px-6 py-1.5 text-xs font-bold rounded-md transition-all ${activeTab === 'categories' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    CATEGORIES
                </button>
                <button 
                    onClick={() => setActiveTab('monthly')}
                    className={`px-6 py-1.5 text-xs font-bold rounded-md transition-all ${activeTab === 'monthly' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    MONTHLY TREND
                </button>
            </div>
            ...
            {activeTab === 'categories' ? (
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2">
                        ...
                        <input type="date" value={dateRange.start} onChange={(e) => handleDateChange('start', e.target.value)} className="border rounded px-2 py-1.5 text-xs text-gray-700 bg-gray-50 outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">To</span>
                        <input type="date" value={dateRange.end} onChange={(e) => handleDateChange('end', e.target.value)} className="border rounded px-2 py-1.5 text-xs text-gray-700 bg-gray-50 outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                </div>
            ) : (
                <div className="flex items-center gap-2">
                    ...
                    <select 
                        ...
                        className="border rounded px-4 py-1.5 text-xs font-bold text-gray-700 bg-gray-50 outline-none focus:ring-2 focus:ring-blue-500 appearance-none cursor-pointer"
                    >
        ...
        {activeTab === 'categories' ? (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
                <div className="lg:col-span-2 bg-white rounded border border-gray-200 shadow-sm p-8">
                    <h3 className="text-gray-400 text-[10px] font-black uppercase tracking-[0.2em] mb-8">Revenue Breakdown</h3>
                    {loading ? (
                ...
                <div className="lg:col-span-1 bg-white rounded border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                    <div className="px-6 py-4 border-b border-gray-100 bg-gray-50"><h3 className="text-gray-700 font-bold text-[10px] uppercase tracking-widest">Category List</h3></div>
            ...
            <div className="bg-white rounded border border-gray-200 shadow-sm p-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
                <h3 className="text-gray-400 text-[10px] font-black uppercase tracking-[0.2em] mb-10">Monthly Performance Trend</h3>
                {loading ? (
                    ...
                    <div className="h-80 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={data.monthlyTrend}>
                                ...
                                <Tooltip cursor={{ fill: '#f9fafb' }} formatter={(val: any) => [`₹${Number(val).toLocaleString()}`, 'Revenue']} contentStyle={{ borderRadius: '4px', border: '1px solid #e5e7eb', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', fontSize: '12px' }} />
