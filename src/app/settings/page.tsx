...
export default function SettingsPage() {
  ...
  const [activeSection, setActiveSection] = useState<'treatments' | 'doctors' | 'expenses'>('treatments');
      ...
      <nav className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
             ...
             <button onClick={() => router.push('/')} className="text-sm text-blue-600 font-bold hover:underline">← Back to Dashboard</button>
          </div>
        </div>
      </nav>
        ...
        <div className="md:hidden flex bg-white border-b border-gray-200 overflow-x-auto">
            {menuItems.map((item) => (
                <button
                    key={item.id}
                    onClick={() => setActiveSection(item.id as any)}
                    className={`flex-1 px-4 py-3 text-[10px] font-black uppercase tracking-widest whitespace-nowrap border-b-2 ${
