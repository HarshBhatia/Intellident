    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8 font-sans text-gray-900">
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">Patient Details</h1>
            <div className="flex flex-wrap gap-2 items-center bg-white p-1 rounded border border-gray-200 shadow-sm overflow-hidden text-gray-900">
                {!isEditing ? (
                    <>
                        ...
                        <button onClick={handleDelete} className="px-3 sm:px-4 py-2 text-red-600 hover:bg-red-50 rounded transition flex items-center gap-2 text-xs sm:text-sm font-medium border border-transparent hover:border-red-100">
                    ...
                    <>
                        ...
                        <button onClick={() => { if (isDirty && !confirm('Discard changes?')) return; setIsEditing(false); setPatient(JSON.parse(JSON.stringify(initialPatient))); setXrayList(initialPatient?.xrays ? JSON.parse(initialPatient.xrays) : []); }} className="px-3 sm:px-4 py-2 text-gray-600 hover:bg-gray-100 rounded transition text-xs sm:text-sm font-medium">
                            Cancel
                        </button>
                    </>
                )}
                <div className="w-px h-6 bg-gray-200 mx-1 hidden sm:block"></div>
                <button onClick={handleBack} className="px-3 sm:px-4 py-2 text-gray-500 hover:text-gray-800 transition text-xs sm:text-sm font-medium flex items-center gap-1">← Back</button>
            </div>
        </div>
        ...
        <form onSubmit={handleSave} className="bg-white shadow rounded overflow-hidden border border-gray-100">
            ...
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div className="col-span-full md:col-span-1 space-y-5">
                    <h3 className="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-bold border-b pb-2">Personal Information</h3>
                    <div className="space-y-4">
                        <div>
                            <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Phone Number</label>
                            {isEditing ? <input name="phone_number" value={patient.phone_number} onChange={handleChange} className="w-full p-2 border rounded focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 bg-gray-50 transition" /> : <div className="p-2 bg-gray-50 rounded text-gray-900">{patient.phone_number || 'N/A'}</div>}
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Age</label>
                                {isEditing ? <input name="age" type="number" value={patient.age} onChange={handleChange} className="w-full p-2 border rounded focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 bg-gray-50 h-[38px]" /> : <div className="p-2 bg-gray-50 rounded text-gray-900 h-[38px] flex items-center font-medium">{patient.age || '0'} Years</div>}
                            </div>
                            <div>
                                <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Gender</label>
                                {isEditing ? <select name="gender" value={patient.gender} onChange={handleChange} className="w-full p-2 border rounded focus:ring-2 focus:ring-blue-500 outline-none transition bg-gray-50 text-gray-900 h-[38px]"><option>Male</option><option>Female</option><option>Other</option></select> : <div className="p-2 bg-gray-50 rounded text-gray-900 h-[38px] flex items-center font-medium">{patient.gender}</div>}
                            </div>
                        </div>
                        <div>
                ...
                <div className="col-span-full md:col-span-1 space-y-5">
                    <h3 className="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-bold border-b pb-2">Treatment</h3>
                    <div className="space-y-4">
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2 mb-3">
                                {patient.treatment_done?.split(',').map(s => s.trim()).filter(Boolean).map((t, idx) => (
                                    <span key={idx} className="bg-blue-50 text-blue-700 text-[10px] font-bold px-2 py-1 rounded border border-blue-100 uppercase tracking-tighter capitalize">
                                        {t}
                        ...
                        <div>
                            ...
                            {isEditing ? <input name="share" value={patient.share || ''} onChange={handleChange} className="w-full p-2 border rounded focus:ring-2 focus:ring-blue-500 outline-none transition text-gray-900 bg-gray-50 h-[38px]" /> : <div className="p-2 bg-gray-50 rounded text-gray-900 h-[38px] flex items-center">{patient.share || 'N/A'}</div>}
                        </div>
                    </div>
                </div>

                <div className="col-span-full md:col-span-1 space-y-5 border-l md:pl-8 border-gray-100 font-sans">
                    <h3 className="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-bold border-b pb-2">Financials</h3>
                    <div className="bg-green-50 rounded p-6 border border-green-100 shadow-inner">
                        <label className="block text-[10px] font-bold text-green-600 uppercase mb-1 tracking-widest text-center font-sans">Total Collection</label>
                        <div className="text-4xl font-black text-green-700 tracking-tighter text-center">₹{patient.amount.toLocaleString()}</div>
                        <div className="mt-4 overflow-hidden rounded border border-green-200">
                            <table className="min-w-full text-[10px] bg-white/50">
                                <thead className="bg-green-100/50 text-green-800 uppercase tracking-widest font-bold font-sans"><tr><th className="px-3 py-1.5 text-left">Date</th><th className="px-3 py-1.5 text-right font-sans">Amt</th></tr></thead>
                                <tbody className="divide-y divide-green-100">
                                    {paymentList.slice(-3).map(p => <tr key={p.id}><td className="px-3 py-1.5 text-green-700">{p.date}</td><td className="px-3 py-1.5 text-right font-bold text-green-800 font-sans">₹{p.amount}</td></tr>)}
                                </tbody>
                            </table>
                        </div>
                    </div>
                ...
                <div className="col-span-full space-y-4 pt-4 border-t border-gray-100">
                    <div className="flex justify-between items-center">
                        <h3 className="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-bold">X-Ray Records</h3>
                        {isEditing && (
                            <label className="cursor-pointer text-blue-600 text-[10px] font-bold tracking-widest hover:underline uppercase">
                                + Add X-Ray
                    ...
                    {xrayList.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                            {xrayList.map((x, idx) => (
                                <div key={x.id} className="group relative border rounded overflow-hidden bg-gray-50 flex flex-col">
                                    ...
                                    {isEditing ? (
                                        <div className="p-2 space-y-1 bg-white border-t">
                                            <input type="date" value={x.date} onChange={(e) => updateXray(idx, 'date', e.target.value)} className="w-full text-[10px] p-1 border rounded text-gray-900" />
                                            <input type="text" placeholder="Note..." value={x.description} onChange={(e) => updateXray(idx, 'description', e.target.value)} className="w-full text-[10px] p-1 border rounded text-gray-900" />
                                        </div>
                                    ) : (
                                        <div className="p-2 text-[10px] text-gray-500 flex flex-col truncate">
                                            <span className="font-bold text-gray-700">{x.date}</span>
                        ...
                        <div className="text-center py-6 bg-gray-50 rounded border border-dashed border-gray-200 text-gray-400 text-xs italic">No X-ray images attached.</div>
                    )}
                </div>

                {/* Payment History */}
                <div className="col-span-full space-y-4 pt-4 border-t border-gray-100 font-sans">
                    <div className="flex justify-between items-center">
                        <h3 className="text-[10px] uppercase tracking-[0.2em] text-gray-400 font-bold">Payment History</h3>
                        {isEditing && !showPaymentForm && (
                            <button type="button" onClick={() => setShowPaymentForm(true)} className="text-blue-600 text-[10px] font-bold hover:underline tracking-widest uppercase font-sans font-bold">+ New Payment</button>
                        )}
                    </div>
                    {isEditing && showPaymentForm && (
                        <div className="bg-gray-50 p-5 rounded border border-blue-100 flex flex-wrap gap-3 items-end animate-in fade-in shadow-inner text-gray-900">
                            <div className="flex-1 min-w-[120px]"><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1 font-sans">Date</label><input type="date" value={newPayment.date} onChange={(e) => setNewPayment({...newPayment, date: e.target.value})} className="w-full p-2 border rounded text-sm text-gray-900 outline-none" /></div>
                            <div className="flex-[2] min-w-[150px]"><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1 font-sans">Purpose</label><input type="text" placeholder="Purpose" value={newPayment.purpose} onChange={(e) => setNewPayment({...newPayment, purpose: e.target.value})} className="w-full p-2 border rounded text-sm text-gray-900 outline-none" /></div>
                            <div className="flex-1 min-w-[100px]"><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1 font-sans font-bold">Mode</label><select value={newPayment.mode} onChange={(e) => setNewPayment({...newPayment, mode: e.target.value})} className="w-full p-2 border rounded text-sm text-gray-900 bg-white outline-none"><option>Cash</option><option>Card</option><option>UPI</option></select></div>
                            <div className="flex-1 min-w-[100px]"><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1 ml-1 font-sans font-bold font-bold font-bold font-sans">Amt (₹)</label><input type="number" value={newPayment.amount} onChange={(e) => setNewPayment({...newPayment, amount: e.target.value})} className="w-full p-2 border rounded text-sm text-gray-900 outline-none" /></div>
                            <div className="flex gap-2"><button type="button" onClick={handleAddPayment} className="bg-blue-600 text-white px-4 py-2 rounded text-[10px] font-bold shadow-sm uppercase h-9 hover:bg-blue-700 transition">Save</button><button type="button" onClick={() => setShowPaymentForm(false)} className="bg-white text-gray-500 border px-3 py-2 rounded text-[10px] font-bold uppercase h-9">✕</button></div>
                        </div>
                    )}
                            ...
                            <tbody className="divide-y divide-gray-50 text-gray-700">
                                ...
                                {paymentList.map((p) => (
                                    <tr key={p.id} className="hover:bg-gray-50 transition">
                                        <td className="px-6 py-3 font-medium font-sans">{p.date}</td><td className="px-6 py-3 capitalize font-sans">{p.purpose}</td><td className="px-6 py-3 font-sans"><span className="bg-gray-100 px-2 py-0.5 rounded text-[10px]">{p.mode}</span></td><td className="px-6 py-3 text-right font-bold text-gray-900 font-mono font-sans">₹{p.amount.toLocaleString()}</td>
                ...
                {/* Notes */}
                <div className="col-span-full grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-gray-100 font-sans font-bold">
                    <div>
                        <label className="block text-[10px] font-bold text-gray-400 uppercase mb-2 tracking-widest font-sans font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold">Medicines</label>
                        {isEditing ? <textarea name="medicine_prescribed" value={patient.medicine_prescribed || ''} onChange={handleChange} className="w-full p-4 border rounded h-32 focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 bg-gray-50 transition text-sm" /> : <div className="p-4 bg-gray-50 rounded text-gray-900 text-sm min-h-[8rem] whitespace-pre-wrap italic text-gray-600 shadow-inner font-sans">{patient.medicine_prescribed || 'None.'}</div>}
                    </div>
                    <div>
                        <label className="block text-[10px] font-bold text-gray-400 uppercase mb-2 tracking-widest font-sans font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold">Notes</label>
                        {isEditing ? <textarea name="notes" value={patient.notes || ''} onChange={handleChange} className="w-full p-4 border rounded h-32 focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 bg-gray-50 transition text-sm" /> : <div className="p-4 bg-gray-50 rounded text-gray-900 text-sm min-h-[8rem] whitespace-pre-wrap italic text-gray-600 shadow-inner font-sans">{patient.notes || 'None.'}</div>}
                    </div>
                </div>

                <div className="col-span-full mt-6 pt-8 border-t border-gray-100 font-sans">
                    <div className="text-center mb-6"><label className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.3em] font-sans font-bold">Dental Odontogram</label></div>
                    ...
                    {!isEditing && patient.tooth_number && <div className="text-center text-[10px] font-bold text-blue-600 mt-2 uppercase tracking-widest font-mono font-sans font-bold">Selected: {patient.tooth_number}</div>}
                </div>
            </div>
        </form>
      </div>
